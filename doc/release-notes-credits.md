Credits Core version *0.15.1* is now available from:

  <https://download.credits.org/credits-0.15.1/>
...
A number of changes to the way Credits Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 creds per kB (0.00001 CRED). This is to prepare for dropping the minimum transaction fee to 0.1 creds per kB (0.0001 CRED) in the next release.
