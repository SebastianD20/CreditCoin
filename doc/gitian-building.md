...
*Setup instructions for a Gitian build of Credits Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Credits
...
- [Building Credits Core](#building-credits-core)
...
Clone the git repositories for credits and Gitian.
...
in the credits repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Credits Core
----------------

To build Credits Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the credits repository.
...
Output from `gbuild` will look something like
    ...
    From https://github.com/credits-project/credits
...
For example:
```bash
URL=https://github.com/thrasher-/credits.git
...
./bin/gbuild --commit credits=${COMMIT} --url credits=${URL} ../credits/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the credits git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../credits/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/credits
SIGPATH=/some/root/path/credits-detached-sigs

./bin/gbuild --url credits=${BTCPATH},signature=${SIGPATH} ../credits/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/credits-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/credits-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/credits-osx-build.assert
```
...
[credits-project/gitian.sigs.ltc](https://github.com/credits-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
